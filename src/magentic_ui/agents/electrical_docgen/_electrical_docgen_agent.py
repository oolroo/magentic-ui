...
class GenDocxUseTemplate(object):
    ...
    def gen_docx(self, variable_dict: Dict[str, Any], output_file_name: str):
        self.template_docx.render(variable_dict)
        self.template_docx.save(self.output_file_path / output_file_name)
...
class ElectrialcalDocGenAgent(BaseChatAgent, Component[ElectrialcalDocGenConfig]):
    ...
    def __init__(
        ...
        *,
        ...
        self.generator = GenDocxUseTemplate(
            os.path.join(self.current_dir_os_path,"docx_template/0_系统部件方案设计说明书.docx"),
            str(self.work_dir),
        )
        self._variable_dict = {}
    ...
    async def on_messages_stream(self, messages: Sequence[BaseChatMessage], cancellation_token: CancellationToken) -> AsyncGenerator[BaseAgentEvent | BaseChatMessage | Response, None]:
        ...
        task_context = "你需要根据用户的问题，判断生成需求说明书所需要的信息是否完善，需要包括需求说明书的名称，例如功率板设计需求说明书；以及作者的名字，例如张三，你需要判断信息是否完善，并回复还需要哪些信息，如果不完善，回复第一句必须包含不完善，例如：用户问题不完善，还需要提供作者信息,请用户输入提供。"
        model_result = None
        async for inference_output in self._call_llm(
            model_client=self.model_client,
            model_client_stream=self.model_client_stream,
            system_messages=self._system_messages + [SystemMessage(content=task_context)],
            model_context = self._model_context,
        ...
        print(f"=========model_result is {self._clean_response_content(str(model_result.content))}")
        if "不完善" in self._clean_response_content(str(model_result.content)):
            yield Response(
            ...
            print("需求信息不完善，请补充信息::",self._clean_response_content(str(model_result.content)))
            return
        
        # second step: generate docx requrment content
        model_result = None
        task_context = "你需要根据用户的问题，提取出需要生成的需求说明说的名称，在回答时不需要回答多余的回答，直接回答需求说明名称，例如功率板设计需求说明书"
        async for inference_output in self._call_llm(
            model_client=self.model_client,
            model_client_stream=self.model_client_stream,
            system_messages=self._system_messages + [SystemMessage(content=task_context)],
        ...
        self._variable_dict["_coverpage_Project_Name"] = self._clean_response_content(str(model_result.content))
...
切记，如果有相关于文档生成的需求，例如需求分析说明书，则通过此agent就可以独立完成任务，而不需要其他agent来完成。也不需要在web上进行联网搜索。'''
    ...
    team = RoundRobinGroupChat([electrial_gendoc, critic_agent], termination_condition=text_termination)
    # Use `asyncio.run(...)` when running in a script.
    result = await team.run(task="帮我生成一个关于电源功率板的需求分析说明书.")
